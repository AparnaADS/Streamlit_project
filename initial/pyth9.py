...
def fetch_payments_for_bills(bill_ids):
    ...
    for bill_id in bill_ids:
        resp = requests.get(f"{API_BASE}/vendorpayments", headers=headers,
...
def fetch_credits_for_bills(bill_ids):
    headers = {"Authorization": f"Zoho-oauthtoken {get_access_token()}"}
    rows = []
    resp = requests.get(f"{API_BASE}/vendorcredits", headers=headers,
                        params={"organization_id": ORG_ID})
    resp.raise_for_status()
    for vc in resp.json().get("vendorcredits", []):
        vc_id = vc["vendor_credit_id"]
        r2 = requests.get(f"{API_BASE}/vendorcredits/{vc_id}/bills", headers=headers,
                          params={"organization_id": ORG_ID})
        r2.raise_for_status()
...
# --- New Fetchers for P&L Metrics ---
def fetch_invoices(start, end):
    headers = {"Authorization": f"Zoho-oauthtoken {get_access_token()}"}
    resp = requests.get(f"{API_BASE}/invoices", headers=headers,
                        params={"organization_id": ORG_ID, "date_start": start, "date_end": end, "status": "paid"})
    resp.raise_for_status()
...
def fetch_creditnotes(start, end):
    headers = {"Authorization": f"Zoho-oauthtoken {get_access_token()}"}
    resp = requests.get(f"{API_BASE}/creditnotes", headers=headers,
                        params={"organization_id": ORG_ID, "date_start": start, "date_end": end})
    resp.raise_for_status()
...
def fetch_customer_payments(start, end):
    headers = {"Authorization": f"Zoho-oauthtoken {get_access_token()}"}
    resp = requests.get(f"{API_BASE}/customerpayments", headers=headers,
                        params={"organization_id": ORG_ID, "date_start": start, "date_end": end})
    resp.raise_for_status()
...
def fetch_bank_accounts():
    headers = {"Authorization": f"Zoho-oauthtoken {get_access_token()}"}
    resp = requests.get(f"{API_BASE}/chartofaccounts", headers=headers,
                        params={"organization_id": ORG_ID})
    resp.raise_for_status()
...
def main():
    ...
    st.dataframe(
        df[["bill_id","vendor_name","due_date","total","total_paid",
             "total_credit","amount_due","aging_bucket"]],
